...
public sealed record HeartbeatMessage(byte SystemId, byte ComponentId, uint CustomMode, byte VehicleType, byte Autopilot, byte BaseMode, byte SystemStatus, byte MavLinkVersion, DateTimeOffset ReceivedAt)
    : MavLinkMessage(SystemId, ComponentId, MessageIds.Heartbeat, ReceivedAt);
